...
import com.eighttea.util.Slugs;
...
public class ProductDaoImpl implements ProductDao {
    ...
    public List<Product> findActiveForShop(Integer categoryId, String keyword, ProductSort sort) {
        ...
        return DbRetry.read(() -> {
            ...
            // Từ khoá KHÔNG lọc bằng SQL LIKE — cột ProductName ở collation accent-sensitive
            // nên "tra" sẽ không khớp "Trà". Lọc bằng Java bên dưới sau khi bỏ dấu cả hai vế.
            ...
            if (hasKeyword) {
                String needle = Slugs.stripDiacritics(keyword.trim()).toLowerCase();
                list.removeIf(p -> p.getName() == null
                        || !Slugs.stripDiacritics(p.getName()).toLowerCase().contains(needle));
            }
            attachVariants(list);
            return list;
        });
