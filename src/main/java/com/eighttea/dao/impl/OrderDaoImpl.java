...
import com.eighttea.dao.OrderDAO;
import com.eighttea.model.CartItem;
import com.eighttea.model.Order;
import com.eighttea.model.OrderAdminFilter;
import com.eighttea.model.OrderDetail;
import com.eighttea.model.OrderTabCounts;
import com.eighttea.model.ProductRevenueRow;
...
public class OrderDaoImpl implements OrderDAO {
    ...
    public int countDoneOrdersByUserId(int userId) {
        ...
        try (Connection con = Database.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ps.setString(2, com.eighttea.util.OrderStatuses.DONE);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return rs.getInt(1);
            }
        } catch (SQLException e) {
    ...
    public int countProcessingOrdersByUserId(int userId) {
        ...
        try (Connection con = Database.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ps.setString(2, com.eighttea.util.OrderStatuses.CONFIRMED);
            ps.setString(3, com.eighttea.util.OrderStatuses.SHIPPING);
            ps.setString(4, com.eighttea.util.OrderStatuses.PENDING_CANCEL);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return rs.getInt(1);
            }
        } catch (SQLException e) {
    ...
    public boolean updateStatusWithValidation(int orderId, String newStatus) throws Exception {
        String normalizedNew = com.eighttea.util.OrderStatuses.normalize(newStatus);
        if (!com.eighttea.util.OrderStatuses.isValid(normalizedNew)) {
        ...
        try (Connection con = Database.getConnection()) {
            ...
            if (!com.eighttea.util.OrderStatuses.canTransition(currentStatus, normalizedNew)) {
    ...
    private FilterSql buildAdminFilterSql(OrderAdminFilter filter) {
        ...
        if (filter != null) {
            ...
            if (filter.getOrderStatus() != null && !filter.getOrderStatus().isBlank()) {
                ...
                params.add(com.eighttea.util.OrderStatuses.normalize(filter.getOrderStatus()));
            }
    ...
    public int placeOrderPayOS(Order order, List<CartItem> cartItems) throws Exception {
        ...
        try {
            ...
            order.setPaymentStatus(com.eighttea.util.PaymentStatuses.PENDING);
    ...
    public void markPayOSLinkFailed(int orderId) throws Exception {
        ...
        try (Connection con = Database.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, com.eighttea.util.PaymentStatuses.FAILED);
            ps.setInt(2, orderId);
            ps.setString(3, com.eighttea.util.PaymentStatuses.PENDING);
            ps.executeUpdate();
        }
    }
